...
- Browser-side Shopify helpers: read [app/utils/app-bridge.js](./app/utils/app-bridge.js) for ID token retrieval, authenticated backend fetches, embedded navigation, and external tab handling. [app/utils/admin-graphql.js](./app/utils/admin-graphql.js) demonstrates App Bridge Direct API access without exposing an Admin access token or routing the request through the app server.
...
6. Create a `shopify.app.toml` file in the root directory copied from [this page](https://shopify.dev/docs/apps/tools/cli/configuration) and replace each value as follows.
    ...
    | `[access.admin].embedded_app_direct_api_access` | `true` | Enables App Bridge Direct API access used by the embedded Admin Link product query. |
    | `[access.admin].direct_api_mode` | `offline` | Matches the app-level offline access behavior previously used by the Admin Link server endpoint. Use `online` to evaluate access through an online token for the current Admin user. |
    ...
    The Admin Link product query uses the [App Bridge Resource Fetching API](https://shopify.dev/docs/api/app-home/apis/authentication-and-data/resource-fetching-api) with the `shopify:admin` URL scheme. Shopify authenticates the Direct API request without exposing an Admin access token to the embedded page. Direct API access is available only while App Bridge is running in the embedded app; server-backed samples and the non-embedded Service Connector continue to use their existing routes.
