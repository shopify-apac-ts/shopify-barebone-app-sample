...
app.js ... Express entry point that serves static assets and hands app routes to React Router. No Shopify CLI generated app template is used.

app/ ... React Router route modules and server helpers for OAuth, embedded app rendering, session token validation, Storefront API, Customer Account API, GraphQL calls, and DB access.

package.json ... app.js used pnpm libaries and scripts for building and running this app.

views/ ... holds HTML rendered by route modules. This is a SPA app which uses index.html only generated by Vite build in frontend.
  ./index.html ... top page view rendered by React Router server code (for code running, this needs replaced with Vite built one).
...
frontend/ ... React code base to be built by Vite for app admin UI. All built code runs at the client = browser as minized JS + CSS.
  ...
  ./src/ ... React source code using App Bridge global APIs and Polaris web components through small local shims.
...
For creating the embedded app frontend, the following contents might help you.
...
3.  If you run it locally, run the following build command (`pnpm install && pnpm run build`). If you use cloud hosting (e.g. Render), use `pnpm install --prod=false` instead to ensure devDependencies (e.g. Vite) are installed even when `NODE_ENV=production`. You can see the details of command definition in `package.json`.
    ...
    (* Both commands install frontend dependencies via pnpm workspaces, build the embedded React app with Vite,
       copy the built assets under public/assets and views/index.html, and build React Router server modules.)
...
# How to update
- For server side update (`app.js`, `app/`, or `views`), run the build command (`pnpm run build`) and start command (`pnpm run start`) again. Some cloud services like Render enable it with `git commit & git push`.
- For frontend update (`frontend` in React with Polaris web components), run the build command (`pnpm run build`) again. If you change the value of `SHOPIFY_API_KEY`, you need to build again. Some cloud services like Render enables it with `git commit & git push`.
